...
from eamodelset.commands.utils import print_error, log
...
def transform(
    ...
    if input_format == output_format:
        print_error('Using the same format for input and output.')
        return
    ...
    if output_format == OutputFormat.JSON:
        ...
        if not Path(DEFAULT_ARCHI_APP_PATH).exists():
            print_error(f"Archi application not found at '{DEFAULT_ARCHI_APP_PATH}'.")
            return
        export_model(input, output, output_format)

    log(f"Transformed model in {output_format.value.upper()} format stored in '{output}'", console, True)

def transform_to_json(input: Path, input_format: InputFormat, output: Path, debug: bool):
    parsed_model = None
    log("Parsing input model...", console, debug)
    ...
    if parsed_model:
        log("Model parsed successfully.", console, debug)
    else:
        print_error("Failed to parse model.")
        return
    
    log('Transforming model to JSON format...', console, debug)
    json_model = parsed_model.get_json()
    with open(output, "w") as outfile: 
        outfile.write(json_model)
    log('Model transformed successfully.', console, debug)
...
def export_archi(input: Path, output: Path):
    try:
        ...
        print_error(f"An error occurred while exporting the model: {e}")
